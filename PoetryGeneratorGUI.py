...
def generate_sonnet(arg):
  try:
    ...
    poem_image = Image.open('calligraphy.png')
  ...
  if filename != "":
    try:
      ...
      while no_of_keywords < 3:
        ...
        if this_keyword != "no person":
          image_keywords.append(this_keyword)
          no_of_keywords = no_of_keywords + 1
        counter = counter + 1
      write_to_log("Key words obtained from image: %s, %s, %s" %(image_keywords[0], image_keywords[1], image_keywords[2]), logger_index)
    except(FileNotFoundError):
      write_to_log("No such file or directory: %s" %filename, logger_index)
      return
...
def bleu_score(arg):
  ...
  write_to_log("BLEU (individual 1-gram) score of Sonnet %d: %.2f" %(arg, average_score), logger_index)
...
menu_about = Menu(menu_bar, name = 'apple')
...
# Frame for log
log_frame = ttk.Frame(main_frame, borderwidth = 8, relief = 'sunken', width = 36, height = 46)
log = Text(log_frame, state = 'disabled', width = 32, height = 46)
log.grid(column = 0, row = 1, pady = 6, sticky = 'n')
log.configure(font = 'times 14', background = 'gray26', foreground = 'snow')
...
sonnet_image = Image.open('calligraphy.png')
...
# Frame for poems
poem_frame = ttk.Frame(main_frame, width = 120, height = 50)
# Sonnet 1
...
poem1_display = Text(poem1_frame, state = 'disabled', width = 52, height = 16)
poem1_display.grid(column = 0, row = 0, padx = 6, pady = 6)
poem1_display.configure(font = 'helvetica 16')
# Sonnet 1 image
poem1_image_frame = ttk.Frame(poem1_frame, width = 68, height = 16)
poem1_image_frame.grid(column = 1, row = 0)
...
poem2_display = Text(poem2_frame, state = 'disabled', width = 52, height = 16)
poem2_display.grid(column = 0, row = 0, padx = 6, pady = 6)
poem2_display.configure(font = 'helvetica 16')
# Sonnet 2 image
poem2_image_frame = ttk.Frame(poem2_frame, width = 68, height = 16)
poem2_image_frame.grid(column = 1, row = 0)
poem2_image = ttk.Label(poem2_image_frame, image = display_sonnet_image)
poem2_image.pack(side = "bottom", fill = "both", expand = "yes")

# Frame for buttons
button_frame = ttk.Frame(main_frame, width = 40, height = 50)
...
ttk.Label(model_frame, textvariable = current_model, font = 'helvetica 14 bold').grid(column = 0, row = 0, pady = 4)
# Number of iterations
ttk.Label(model_frame, text = "Number of iterations:").grid(column = 0, row = 1, sticky = 'nw', pady = (6, 2))
# Radio frame
radio_frame = ttk.Frame(model_frame)
radio_frame.grid(column = 0, row = 2, pady = 6)
...
load_buttons_frame.grid(column = 0, row = 1, pady = 6)
# Load buttons
ttk.Button(load_buttons_frame, text = "Load RNN Model", command = load_rnn).grid(column = 0, row = 0, padx = 4, pady = 4)
ttk.Button(load_buttons_frame, text = "Load GRU Model", command = load_gru).grid(column = 1, row = 0, padx = 4, pady = 4)
ttk.Button(load_buttons_frame, text = "Load RNN Bigram", command = load_rnn_bigram).grid(column = 0, row = 1, padx = 4, pady = 4)
ttk.Button(load_buttons_frame, text = "Load GRU Bigram", command = load_gru_bigram).grid(column = 1, row = 1, padx = 4, pady = 4)
ttk.Button(load_buttons_frame, text = "Open Custom RNN", command = load_custom_rnn).grid(column = 0, row = 2, padx = 4, pady = 4)
ttk.Button(load_buttons_frame, text = "Open Custom GRU ", command = load_custom_gru).grid(column = 1, row = 2, padx = 4, pady = 4)
# Separator
ttk.Separator(button_frame, orient = HORIZONTAL).grid(column = 0, row = 2, pady = 8, sticky = 'ew')

# Sonnet 1
# Frame for sonnet 1
sonnet1_frame = ttk.Frame(button_frame)
sonnet1_frame.grid(column = 0, row = 3)
ttk.Label(sonnet1_frame, text = "Sonnet 1", font = 'helvetica 14 bold').grid(column = 0, row = 0, pady = 4)
# Theme
theme1_frame = ttk.Frame(sonnet1_frame)
theme1_frame.grid(column = 0, row = 1)
ttk.Label(theme1_frame, text = "Theme:").grid(column = 0, row = 0, sticky = 'nw', pady = 4)
theme1 = StringVar()
theme1_entry = ttk.Entry(theme1_frame, width = 24, textvariable = theme1).grid(column = 1, row = 0, pady = 4)
...
generate_sonnet1_button.grid(column = 0, row = 2, pady = 4)
...
generate_sonnet1_image_button.grid(column = 0, row = 3, pady = 4)
ttk.Button(sonnet1_frame, text = "Random Shakespeare's Sonnet", command = lambda: get_example_poem(1)).grid(column = 0, row = 4, pady = 4)
# BLEU Score
bleu1_frame = ttk.Frame(sonnet1_frame)
bleu1_frame.grid(column = 0, row = 5)
ttk.Button(bleu1_frame, text = "BLEU Score", command = lambda: bleu_score(1)).grid(column = 0, row = 0, padx = 4, pady = 4)
ttk.Button(bleu1_frame, text = "BLEU Score (n-gram)", command = lambda: bleu_ngram_score(1)).grid(column = 1, row = 0, padx = 4, pady = 4)
# Copy / Clear Sonnet
copy_clear1_frame = ttk.Frame(sonnet1_frame)
copy_clear1_frame.grid(column = 0, row = 6)
ttk.Button(copy_clear1_frame, text = "Copy to Clipboard", command = lambda: copy_to_clipboard(1)).grid(column = 0, row = 0, padx = 4, pady = 4)
ttk.Button(copy_clear1_frame, text = "Clear Sonnet", command = lambda: clear_sonnet(1)).grid(column = 1, row = 0, padx = 4, pady = 4)
# Separator
ttk.Separator(button_frame, orient = HORIZONTAL).grid(column = 0, row = 4, pady = 8, sticky = 'ew')

# Sonnet 2
# Frame for sonnet 2
sonnet2_frame = ttk.Frame(button_frame)
sonnet2_frame.grid(column = 0, row = 5)
ttk.Label(sonnet2_frame, text = "Sonnet 2", font = 'helvetica 14 bold').grid(column = 0, row = 0, pady = 4)
# Theme
theme2_frame = ttk.Frame(sonnet2_frame)
theme2_frame.grid(column = 0, row = 1)
ttk.Label(theme2_frame, text = "Theme:").grid(column = 0, row = 0, sticky = 'nw', pady = 4)
theme2 = StringVar()
theme2_entry = ttk.Entry(theme2_frame, width = 24, textvariable = theme2).grid(column = 1, row = 0, pady = 4)
# Generate sonnet button
generate_sonnet2_button = ttk.Button(sonnet2_frame, text = "Generate Sonnet", state = 'disabled', command = lambda: generate_sonnet(2))
generate_sonnet2_button.grid(column = 0, row = 2, pady = 4)
...
generate_sonnet2_image_button.grid(column = 0, row = 3, pady = 4)
ttk.Button(sonnet2_frame, text = "Random Shakespeare's Sonnet", command = lambda: get_example_poem(2)).grid(column = 0, row = 4, pady = 4)
# BLEU Score
bleu2_frame = ttk.Frame(sonnet2_frame)
bleu2_frame.grid(column = 0, row = 5)
ttk.Button(bleu2_frame, text = "BLEU Score", command = lambda: bleu_score(2)).grid(column = 0, row = 0, padx = 4, pady = 4)
ttk.Button(bleu2_frame, text = "BLEU Score (n-gram)", command = lambda: bleu_ngram_score(2)).grid(column = 1, row = 0, padx = 4, pady = 4)
# Copy / Clear Sonnet
copy_clear2_frame = ttk.Frame(sonnet2_frame)
copy_clear2_frame.grid(column = 0, row = 6)
ttk.Button(copy_clear2_frame, text = "Copy to Clipboard", command = lambda: copy_to_clipboard(2)).grid(column = 0, row = 0, padx = 4, pady = 4)
ttk.Button(copy_clear2_frame, text = "Clear Sonnet", command = lambda: clear_sonnet(2)).grid(column = 1, row = 0, padx = 4, pady = 4)
# Separator
ttk.Separator(button_frame, orient = HORIZONTAL).grid(column = 0, row = 6, pady = 6, sticky = 'ew')

# Others
ttk.Button(button_frame, text = "Clear Log", command = clear_log).grid(column = 0, row = 7, pady = 4)
ttk.Button(button_frame, text = "Exit", command = exit_program).grid(column = 0, row = 8, pady = 4)

# Outer frames coordinates
log_frame.grid(column = 0, row = 0, sticky = 'n', padx = 12, pady = 6)
poem_frame.grid(column = 1, row = 0, padx = 12, pady = 6)
button_frame.grid(column = 2, row = 0, sticky = 'ns', padx = 12, pady = 6)
